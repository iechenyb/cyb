...
Monitor()
{
  ...
  then
	...
	then
        echo "[info]response code is $TomcatServiceCode,tomcat start normall......"
    else
        echo "[error]tomcat exception ......state is $TomcatServiceCode,please look log file at $GetPageInfo"
        echo "[error]page is wrong ,try to restart tomcat"
        kill -9 $TomcatID
        sleep 3
        rm -rf $TomcatCache
        $StartTomcat
